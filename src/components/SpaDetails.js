      ...
      if (this.props.currentTrip.spas === null || this.props.currentTrip.spas.length === 0) {
        return(
          <div className="details-section col-lg-12 mb-4 mt-4">
            <h3>Beauty & Spas:</h3>
            <button className="details-find-btn btn btn-info" onClick={ () => this.handleSpaSearch() } ><i className="fa fa-search" aria-hidden="true"></i> Discover Beauty & Spas</button>
          </div>
        )
      } else {

        const spas = this.props.currentTrip.spas.map( (business, index) => {
